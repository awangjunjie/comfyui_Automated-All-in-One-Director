...
export function mountPromptEnhancerPanel(editor, parentEl) {
    ...
    pe.enhanceOneTarget = async (segmentIndex, cfg, activeBtn, label) => {
        ...
        if (result.ok) {
            const text = (taskKey === "fl2v" || taskKey === "fl_chain") ? stripFl2vPromptBody(result.text) : result.text;
    ...
    pe.handleServerEnhanced = (payload) => {
        ...
        if (editor.isFl2vMode?.() || ["fl2v", "fl_chain"].includes(resolveTaskKey(editor.getTaskKey?.() || ""))) {
