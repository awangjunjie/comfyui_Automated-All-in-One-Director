...
def enrich_timeline_dict(timeline: dict) -> dict:
    ...
    is_prompt_batch = (
        ...
        or task_key in ("t2v", "i2v", "r2v", "novel", "film", "m2v", "fl2v")
        or len(segs) > 1
    )
    ...
    if task_key == "m2v":
        ...
        # Drop stale 2D/3D medium tags before appending current desk.style so old looks don't stick.
        desk = timeline.get("desk") or {}
        desk_style = str(desk.get("style") or "").strip()
        if desk_style:
            try:
                from .local_director_runtime import reconcile_prompt_with_desk_style

                base_prompt = reconcile_prompt_with_desk_style(base_prompt, desk_style)
            except Exception:
                pass
        ...
        if desk_suffix:
            for line in desk_suffix.splitlines():
                ...
                if line and line not in base_prompt:
                    # Style line already reconciled above; still append soundscape/music/notes.
                    if desk_style and line == desk_style:
                        continue
