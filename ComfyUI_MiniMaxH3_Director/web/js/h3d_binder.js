...
/** m2v：隐藏剧本设定；只保留素材 / 动作视频 / 出库。novel：小说导入 → 全局参考 → 章节分镜 → 出库。 */
export function updateBinderStepCopy(editor) {
    ...
    const taskKey =
        (typeof editor.getTaskKey === "function" && editor.getTaskKey()) ||
        editor.taskTypeWidget?.value ||
        "";
    const isNovel = String(taskKey).startsWith("novel") || String(taskKey).includes("novel —")
        || String(taskKey).startsWith("film") || String(taskKey).includes("film —");
    const isFilm = String(taskKey).startsWith("film") || String(taskKey).includes("film —");
    ...
    root.classList.toggle("h3d-binder-novel", !!isNovel);
    root.classList.toggle("h3d-binder-film", !!isFilm);
    ...
    if (isM2v) {
        ...
        if (isFilm) {
            setStep("bible", "剧本导入", "剧本 · 定妆 · 音视频库");
            setStep("shots", "分镜核对", "按镜挂乐/动作 · 剪辑");
            setStep("output", "成片出库", "分批 Queue · 续跑");
            setPanel("bible", "01 · 剧本导入", "电影：整份剧本直导 → 定妆/音频库/动作库 → 追加分镜 → 按镜挂乐与动作剪辑 → 继续生成");
            setPanel("shots", "02 · 分镜核对", "核对分镜与挂图；在分镜卡按镜选用配乐/动作视频，填起止秒剪辑放置（不自动灌全局）");
            setPanel("output", "03 · 成片出库", "可手动 Queue；推荐用面板「④ 继续生成」分批续跑");
        } else {
            setStep("bible", "小说导入", "导入 · 资产 · 一键跑全书");
            setStep("shots", "章节分镜", "核对分镜卡 · 参考图");
            setStep("output", "成片出库", "Queue 本章");
            setPanel("bible", "01 · 小说导入", "短剧：本章分镜 → 挂图 → 装载，或「一键跑全书」（无追加/续跑）；工台只配 LLM");
            setPanel("shots", "02 · 章节分镜", "核对本章分镜与按镜参考图，确认后去出库 Queue");
            setPanel("output", "03 · 成片出库", "设置分辨率并 Queue 生成本章");
        }
    } else {
...
export function syncBinderContent(editor) {
    ...
    if (bibleBody && editor.novelPanel) _move(editor.novelPanel, bibleBody, true);
