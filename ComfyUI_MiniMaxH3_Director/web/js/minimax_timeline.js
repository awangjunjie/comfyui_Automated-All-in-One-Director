...
import {
    mountNovelPanel,
    updateNovelPanelVisibility,
    slimNovelForTimeline,
    ensureNovelTimeline,
} from "./minimax_novel.js";
import {
    FL2V_STYLES,
    bindFl2vEvents,
...
class MiniMaxH3DirectorEditor {
    ...
    buildTimelinePayload() {
        ...
        if (this.isImageBatch()) {
            ...
            if (batchBody.novel) {
                batchBody.novel = slimNovelForTimeline(batchBody.novel);
            } else {
                ensureNovelTimeline(batchBody);
            }
    ...
    /** Pull latest prompt + duration from batch cards into timeline.segments (before Queue). */
    harvestBatchPrompts() {
        ...
        cards.forEach((card, i) => {
            ...
            const secEl = card.querySelector('input[data-f="durationSec"]')
                || card.querySelector(".h3d-batch-fc input[type='number']");
            if (!secEl || secEl.value === "") return;
            let sec = parseFloat(secEl.value);
            if (!Number.isFinite(sec)) sec = defaultDurationSec(key);
            const lo = minDurationSec();
            sec = Math.max(lo, sec);
    ...
    buildDOM() {
        ...
        try {
            mountNovelPanel(this);
            updateNovelPanelVisibility(this);
        } catch (err) {
            console.warn("[MiniMaxH3Director] novel panel mount failed:", err);
        }
    }
    ...
    isR2vBatch() {
        // Pure r2v material-group cards (not m2v / novel — those have no r2v media-track step).
    ...
    applyTaskLayout(prevMode) {
        ...
        const taskKey = this.getTaskKey();
        // novel：生成管线同 r2v，但分镜清单/工具栏走图生视频（i2v）样式
    ...
    updateSegmentContinuityUI() {
        ...
        if (support) {
            ...
            if (key === "fl_chain") {
                ...
                this.segmentContinuityCb.title = (key === "novel" || key === "film")
                    ? (key === "film"
                        ? "电影推荐开启：同场景时上镜末帧硬锁为本镜首帧；换场时不强锁背景"
                        : "短剧推荐开启：同场景时上镜末帧硬锁为本镜首帧（人物/场景参考图仍保留）；换场时不强锁背景")
                    : "";
    ...
    commit(skipRender = false, { syncTimeline = true } = {}) {
        // Harvest card fields BEFORE any re-render so hand-tuned seconds aren't wiped
        this.harvestBatchPrompts?.();
        this.syncFromWidgets();
        this.normalizeSegments();
    ...
    updateNfe8AccelUI() {
        ...
        const key = resolveTaskKey(this.getTaskKey?.() || this.taskTypeWidget?.value || "");
        const show = ["t2v", "i2v", "fl2v", "fl_chain", "r2v", "novel", "film", "m2v"].includes(key);
        if (wrap) {
