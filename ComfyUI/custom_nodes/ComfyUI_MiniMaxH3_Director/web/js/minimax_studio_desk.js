...
function _looksPromptBlob(v) {
    if (typeof v !== "string") return false;
    const s = v.trim();
    if (s.length < 24) return false;
    if (_looksGgufPath(s) || _looksDirectorMode(s)) return false;
    if (/\.safetensors$/i.test(s)) return false;
    // Multimodal / storyboard dumps commonly land in the wrong FLOAT slot after widget inserts.
    return (
        s.includes("\n")
        || /integrated_multimodal_description|overall_soundscape|<Picture\s+\d+/i.test(s)
        || /第[一二三四五六七八九十\d]+段/.test(s)
        || s.length > 80
    );
}

function _looksLoraName(v, comboValues) {
    if (v == null) return false;
    const s = String(v).trim();
    if (!s || s === "none") return true;
    if (_looksPromptBlob(s) || _looksGgufPath(s) || typeof v === "boolean" || typeof v === "number") {
        return false;
    }
    if (Array.isArray(comboValues) && comboValues.length && comboValues.includes(s)) return true;
    return /\.(safetensors|pt|ckpt)$/i.test(s) || /tutu|20to8|nfe|lora/i.test(s);
}
...
export function repairDirectorStudioWidgets(node) {
    ...
    for (const v of allVals) {
        ...
        if (_looksPromptBlob(v)) promptBlob = v;
        if (typeof v === "number" && Number.isFinite(v)) {
            ...
            if (v >= -2 && v <= 2) strengthHint = strengthHint ?? v;
        }
    }
    ...
    // --- NFE8 widgets inserted before studio: old workflows shift brief → strength ---
    const nfeStrW = byName("nfe8_lora_strength");
    const nfeNameW = byName("nfe8_lora_name");
    const nfeEnW = byName("nfe8_accel_enable");
    const briefW = byName("local_director_brief");
    if (nfeStrW) {
        const raw = nfeStrW.value;
        const num = typeof raw === "number" ? raw : parseFloat(raw);
        const broken = !Number.isFinite(num) || _looksPromptBlob(raw) || typeof raw === "boolean";
        if (broken) {
            if (_looksPromptBlob(raw)) {
                const briefEmpty = !briefW || !String(briefW.value || "").trim()
                    || _looksGgufPath(briefW.value)
                    || _looksDirectorMode(briefW.value)
                    || typeof briefW.value === "boolean";
                if (briefEmpty) set("local_director_brief", String(raw));
            } else if (promptBlob && briefW && !String(briefW.value || "").trim()) {
                set("local_director_brief", String(promptBlob));
            }
            const fallback = Number.isFinite(strengthHint) ? strengthHint : 0.8;
            set("nfe8_lora_strength", Math.max(-2, Math.min(2, fallback)));
        } else if (typeof raw !== "number") {
            set("nfe8_lora_strength", Math.max(-2, Math.min(2, num)));
        }
    }
    if (nfeNameW && !_looksLoraName(nfeNameW.value, nfeNameW.options?.values)) {
        set("nfe8_lora_name", "none");
    }
    if (nfeEnW && typeof nfeEnW.value !== "boolean") {
        // Misaligned bools/strings must not stay in the enable slot
        set("nfe8_accel_enable", false);
    }
...
/**
 * Wipe sticky creative cache so a new video can start clean (no restart needed).
 * Keeps LLM / API / skill wiring; clears prompts, bible, desk look, and group texts.
 */
export function clearGlobalCreativeCache(editor, { clearSegments = true } = {}) {
    if (!editor?.timeline) return { ok: false, error: "无时间线" };
    ensureTimelineStudio(editor.timeline);
    const tl = editor.timeline;
    const deskEl = editor.studioDesk;

    tl.global = tl.global && typeof tl.global === "object" ? tl.global : {};
    tl.global.prompt = "";
    if (editor.globalPrompt) editor.globalPrompt.value = "";
    if (editor.globalPromptWidget) editor.globalPromptWidget.value = "";
    ...
    tl.continuity = {
        ...
        locations: "",
        props: "",
        inject: tl.continuity?.inject !== false,
    };

    tl.desk = tl.desk && typeof tl.desk === "object" ? tl.desk : {};
    tl.desk.style = "";
    tl.desk.soundscape = "";
    tl.desk.music = "";
    tl.desk.image_director_note = "";
    const td = normalizeTextDirector(tl.desk.text_director);
    td.brief = "";
    td.style_refresh = true;
    tl.desk.text_director = td;

    if (clearSegments && Array.isArray(tl.segments)) {
    ...
    // Novel / film chapter globalPrompt in memory (+ product buckets)
    const wipeNovelGp = (novel) => {
        if (!novel || typeof novel !== "object") return;
        for (const ch of novel.chapters || []) {
            if (ch && typeof ch === "object") ch.globalPrompt = "";
        }
    };
    wipeNovelGp(tl.novel);
    const bucket = tl.novelByProduct;
    if (bucket && typeof bucket === "object") {
        wipeNovelGp(bucket.novel);
        wipeNovelGp(bucket.film);
    }

    clearIdirPromptDraft(editor);

    const setVal = (sel, val) => {
        const el = deskEl?.querySelector?.(sel);
        if (el) el.value = val ?? "";
    };
    setVal('[data-r="ld-global-prompt"]', "");
    setVal('[data-r="ld-brief"]', "");
    setVal('[data-r="cont-characters"]', "");
    setVal('[data-r="cont-locations"]', "");
    setVal('[data-r="cont-props"]', "");
    setVal('[data-r="desk-style"]', "");
    setVal('[data-r="desk-soundscape"]', "");
    setVal('[data-r="desk-music"]', "");
    setVal('[data-r="shot-preview"]', "");
...
/** Strip sticky 2D/3D/medium look tags from a prompt so new desk.style can win. */
function stripStaleMediumStyle(text) {
    let s = String(text || "");
    if (!s) return "";
    s = s.replace(
        /(?:2\s*[DdＤｄ]|3\s*[DdＤｄ]|二维|三维|手绘(?:风|风格)?|赛璐璐|二次元|三次元|皮克斯风?|迪士尼风?|定格动画|黏土动画|写实(?:电影|摄影|风格)?|动漫风?|动画电影|卡通风?|CG(?:动画|渲染)?|油画风?|水彩风?|像素风?)(?:\s*(?:动画|风格|质感|渲染|电影|画面|短片))?/g,
        "",
    );
...
function normalizeTextDirector(td) {
    ...
    out.style_refresh = !!out.style_refresh;
...
export function mountStudioDesk(editor) {
    ...
    desk.innerHTML = `
      <nav class="h3d-desk-nav" data-r="desk-tabs" aria-label="导演工台分区">
        ...
        <button type="button" class="hidden" data-tab="novel-llm" title="小说模式：推理 API 与整体风格">推理</button>
      </nav>
      <div class="h3d-desk-main">
      <div class="h3d-studio-desk-head" style="display:flex;align-items:center;gap:8px">
        <div data-r="desk-toggle" class="h3d-studio-desk-head-toggle" style="display:flex;flex:1;align-items:center;justify-content:space-between;gap:8px;cursor:pointer;min-width:0">
          <b data-r="desk-title">H3 导演工台 · 连续性 / 声景 / 提示词</b>
          <span class="h3d-meta">点击折叠</span>
        </div>
        <button type="button" class="h3d-btn" data-a="ld-clear-global-cache" style="flex-shrink:0" title="清空全局提示词、创意简述、连续性、整体风格/声景，以及各组历史提示词，便于开始新创作（不改 LLM 配置）">清除全局缓存</button>
      </div>
      <div class="h3d-studio-desk-body">
        ...
        <div class="h3d-studio-page" data-page="global">
          <div class="h3d-studio-grid h3d-studio-grid-3 h3d-studio-grid-tall" data-r="desk-style-grid">
        ...
        <div class="h3d-studio-page" data-page="director">
          <div data-r="ld-novel-move-backend">
          <div class="h3d-studio-row">
            ...
            <div class="h3d-meta" data-r="ld-backend-hint" style="flex:1;font-size:11px;color:#8b93a7;align-self:center">
              本地需 LLM Text Processor + GGUF；云端支持 Ollama / 智谱 / DeepSeek / OpenAI Compatible
            </div>
          </div>
          ...
          <div data-r="ld-cloud-panel" class="hidden">
            ...
            <div class="h3d-studio-row">
              <div class="h3d-studio-field" style="flex:2"><label>云端模型</label>
                <input type="text" data-r="ld-api-model" list="ld-api-model-list" placeholder="deepseek-v4-flash / glm-5.2…">
              ...
              <div class="h3d-studio-field" style="flex:1.2" data-r="ld-api-key-wrap"><label>API Key</label>
                <input type="password" data-r="ld-api-key" placeholder="DeepSeek / 智谱必填" autocomplete="off">
              </div>
          ...
          <div class="h3d-studio-row">
            ...
            <div class="h3d-studio-field" style="flex:1.4"><label>风格 Skill</label>
              <select data-r="ld-skill" title="H3 风格技能精简版，扩写时注入；小说模式默认「小说短剧」">
                <option value="none">通用（H3 规范）</option>
                <option value="novel-short-drama">小说短剧</option>
                <option value="novel-film">电影模式</option>
            ...
            <div class="h3d-studio-field" data-r="ld-shot-count-wrap" style="width:88px"><label>拆分镜数</label>
              <input type="number" data-r="ld-shot-count" min="1" step="1" value="2" title="「故事 → N 组分镜」的固定组数；超过 ${SHOT_COUNT_WARN_THRESHOLD} 会提示确认">
            </div>
          ...
          <div class="h3d-studio-grid h3d-studio-grid-2 h3d-studio-grid-tall" data-r="ld-story-prompt-grid">
            ...
            <div class="h3d-studio-field">
              <div class="h3d-studio-row" style="justify-content:space-between;align-items:center;gap:6px;margin-bottom:0">
                ...
                <span class="h3d-studio-row" style="gap:4px;flex-wrap:wrap">
                  ...
                  <button type="button" class="h3d-btn" data-a="ld-clear-global-cache" title="清空全局提示词、简述、连续性、风格声景与各组历史提示词">清除全局缓存</button>
                </span>
              </div>
          ...
          <div class="h3d-studio-row" data-r="ld-shot-range-row" title="仅「故事 → 自动分镜」使用：镜数与单镜时长范围（无硬上限；超过软阈值弹窗确认）">
            <div class="h3d-studio-field" style="width:76px"><label>镜数下限</label>
              <input type="number" data-r="ld-shot-min" min="1" step="1" value="2">
            </div>
            <div class="h3d-studio-field" style="width:76px"><label>镜数上限</label>
              <input type="number" data-r="ld-shot-max" min="1" step="1" value="8" title="超过 ${SHOT_COUNT_WARN_THRESHOLD} 会提示确认">
            </div>
            <div class="h3d-studio-field" style="width:88px"><label>单镜秒下限</label>
              <input type="number" data-r="ld-dur-min" min="0.5" step="0.5" value="2">
            </div>
            <div class="h3d-studio-field" style="width:88px"><label>单镜秒上限</label>
              <input type="number" data-r="ld-dur-max" min="0.5" step="0.5" value="12" title="超过 ${DURATION_SEC_WARN_THRESHOLD}s 会提示确认">
            </div>
          </div>
          ...
          <div class="h3d-studio-row">
            ...
            <button type="button" class="h3d-btn" data-a="ld-clear-global-cache" title="新创作前一键清空历史全局设定与各组提示词（不改 LLM）">一键清除全局缓存</button>
          </div>
        ...
        <div class="h3d-studio-page hidden" data-page="novel-llm">
          <div class="h3d-meta" style="font-size:11px;color:#8b93a7;line-height:1.45;margin-bottom:8px">
            分镜、挂图、装载请用上方「小说短剧 / 电影模式」面板；此处仅配置 LLM（提取资产/本章分镜）与整体风格声景。
          </div>
          <div data-r="novel-llm-style-host"></div>
          <div data-r="novel-llm-backend-host" style="margin-top:10px;border-top:1px solid rgba(255,255,255,.08);padding-top:10px"></div>
        </div>
      </div>
      </div>`;
    ...
    desk.querySelectorAll("[data-tab]").forEach((btn) => {
        btn.onclick = () => {
            ...
            desk.querySelectorAll("[data-page]").forEach((p) => {
                const on = p.dataset.page === btn.dataset.tab;
    ...
    // Homes for novel-mode relocate (style grid + LLM backend block)
    editor._novelStyleHome = desk.querySelector('[data-r="desk-style-grid"]')?.parentNode || null;
    editor._novelBackendHome = desk.querySelector('[data-r="ld-novel-move-backend"]')?.parentNode || null;
    ...
    bindField('[data-r="desk-style"]', (el) => {
        ...
        const prev = String(editor.timeline.desk.style || "");
        editor.timeline.desk.style = el.value;
        if (String(el.value || "").trim() !== prev.trim()) {
            markStyleRefresh();
        }
    });
    ...
    const updateDirectorBackendUI = () => {
        ...
        if (keyWrap) {
            keyWrap.style.display =
                (fmt === "智谱 GLM" || fmt === "DeepSeek" || fmt === "OpenAI Compatible") ? "" : "none";
        }
        const hint = q('[data-r="ld-backend-hint"]');
        if (hint) {
            hint.textContent = backend === "cloud"
                ? "云端：Ollama / 智谱 / DeepSeek / OpenAI Compatible。DeepSeek 填 Key，模型用 deepseek-v4-flash / deepseek-v4-pro（旧 deepseek-chat 会自动映射）"
    ...
    const markStyleRefresh = () => {
        const td = ensureTextDirector();
        td.style_refresh = true;
        // Soft-scrub sticky medium tags from the global box so next expand/Queue won't keep 2D after switching to 3D
        const cur = harvestLdGlobalPrompt()
            || String(editor.timeline.global?.prompt || "").trim();
        if (cur) {
            const scrubbed = stripStaleMediumStyle(cur);
            if (scrubbed !== cur) applyGlobalPrompt(scrubbed, { skipEmpty: false });
        }
        editor.commit?.(false, { syncTimeline: true });
    };

    const clearStyleRefresh = () => {
        const td = ensureTextDirector();
        td.style_refresh = false;
    };
    ...
    const deskPreserveFields = () => {
        ...
        let fromUi = String(q('[data-r="ld-global-prompt"]')?.value || "").trim();
        let fromTl = String(editor.timeline.global?.prompt || "").trim();
        let gp = fromUi || fromTl;
        if (td.style_refresh && gp) gp = stripStaleMediumStyle(gp) || gp;
        return {
            global_prompt: gp,
            desk_style: String(desk.style || "").trim(),
            desk_soundscape: String(desk.soundscape || "").trim(),
            desk_music: String(desk.music || "").trim(),
            force_style_refresh: !!td.style_refresh,
        };
    };
    ...
    const commonBody = (extra = {}) => {
        ...
        const body = {
            ...
            style_refresh: !!td.style_refresh,
            ...deskPreserveFields(),
            ...extra,
        };
        if (backend === "cloud") {
    ...
    q('[data-r="ld-api-format"]').onchange = () => {
        ...
        // Switch format → fill recommended model when empty or still a known default.
        if (modelEl && (!modelEl.value.trim() || Object.values(LD_API_DEFAULTS).some((d) => d.model === modelEl.value.trim()))) {
            modelEl.value = defs.model;
        }
        updateDirectorBackendUI();
        persistTextDirectorFromUI();
    ...
    const onClearGlobalCache = async (e) => {
        e?.preventDefault?.();
        e?.stopPropagation?.();
        const ok = window.confirm(
            "清除全局缓存？\n\n将清空：全局提示词、创意简述、连续性、整体风格/声景/配乐、各组提示词、参考图草稿，以及小说/电影章节的全局提示词缓存。\n\n不改动：LLM 后端/模型/API、风格 Skill、参考图文件、分镜结构。\n\n便于开始新的视频创作。",
        );
        if (!ok) return;
        setStatus(statusEl, "正在清除全局缓存…");
        const result = clearGlobalCreativeCache(editor, { clearSegments: true });
        if (!result?.ok) {
            setStatus(statusEl, result?.error || "清除失败", "err");
            return;
        }
        const pid = String(editor.timeline?.novel?.projectId || "").trim();
        if (pid) {
            try {
                const res = await api.fetchApi("/minimax/director/novel/clear_global_cache", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        projectId: pid,
                        chapterId: editor.timeline?.novel?.currentChapterId || "",
                        allChapters: true,
                    }),
                });
                const data = await res.json().catch(() => ({}));
                if (data?.novel) {
                    editor.timeline.novel = data.novel;
                }
            } catch (err) {
                console.warn("novel clear_global_cache:", err);
            }
        }
        setStatus(statusEl, "已清除全局缓存，可直接填写新故事开始创作", "ok");
    };
    desk.querySelectorAll('[data-a="ld-clear-global-cache"]').forEach((btn) => {
        btn.onclick = (ev) => { void onClearGlobalCache(ev); };
    });
    editor.clearGlobalCreativeCache = () => clearGlobalCreativeCache(editor, { clearSegments: true });
    editor.clearGlobalCreativeCacheAsync = onClearGlobalCache;
    if (modelSel) {
    ...
    const doStoryAuto = async () => {
        ...
        if (!confirmHighShotOrDuration({
            ...
            setStatus(statusEl, "已取消自动分镜", "err");
            return;
        }
    ...
    const doExpand = async (toSegment) => {
        ...
        const story = q('[data-r="ld-brief"]')?.value?.trim() || "";
        const td = ensureTextDirector();
        const styleRefresh = !!td.style_refresh;
        let existingGlobal = harvestLdGlobalPrompt()
            || String(editor.timeline.global?.prompt || "").trim();
        if (styleRefresh && existingGlobal) {
            existingGlobal = stripStaleMediumStyle(existingGlobal) || existingGlobal;
        }
        let brief = story;
        ...
        if (toSegment) {
            ...
            if (styleRefresh && brief) brief = stripStaleMediumStyle(brief) || brief;
            if (seg?.durationSec) duration = Number(seg.durationSec) || duration;
        } else {
            // 扩写全局：优先用创意简述（故事微调后以此为准）；无简述时再扩写本框
            brief = story || existingGlobal;
        }
        if (!brief) {
            setStatus(statusEl, "请先在「全局提示词」或「创意简述」中填写内容", "err");
            return;
        }
        const forceStyle = styleRefresh || !!(story && !toSegment && existingGlobal);
        setStatus(statusEl, toSegment
            ? `提示词导演扩写当前组…（${taskKey}/${mode}）`
            : `提示词导演扩写全局…（${forceStyle ? "按新风格重写画风，" : ""}保留场景位置/声景设定）`);
        try {
            const body = commonBody({
                ...
                force_style_refresh: forceStyle,
            });
            if (!body.model) return;
            const res = await api.fetchApi("/minimax/director/local_expand", {
                method: "POST",
    ...
    editor.updateNovelDeskMode = () => updateNovelDeskMode(editor);
    syncSegStudioFields(editor);
...
export function syncLocalDirectorForTask(editor) {
    ...
    // novel/film：分镜走章节面板，工台推理页不展示拆镜控件
    const isNovelLike = ["novel", "film"].includes(resolveTaskKey(taskKey));
    const hideSplit = globalScope || isNovelLike;
    ...
    if (hint) {
        ...
        if (key === "m2v") {
            ...
            groupNote = "小说短剧 · 用上方面板「本章分镜/一键跑全书」；此处只配 LLM 与风格";
        } else if (key === "film") {
            groupNote = "电影模式 · 用上方面板「追加分镜/继续生成」；此处只配 LLM 与风格";
        } else if (globalScope) {
    ...
    if (flow) {
        ...
        if (key === "m2v") {
            ...
            flow.textContent = "小说短剧：导入 → 资产 → 本章分镜 → 挂图 → 装载 Queue，或「一键跑全书」（无追加分镜/断点续跑）";
        } else if (key === "film") {
            flow.textContent = "电影长片：整份剧本直导（不切章）→ 定妆/音频/动作视频 → 追加分镜 → 挂图 →「继续生成」（有动作视频的镜走动作迁移）";
        } else {
            flow.textContent = globalScope
...
function _moveDeskBlock(el, dest) {
    if (!el || !dest || el.parentNode === dest) return;
    dest.appendChild(el);
}

/**
 * Novel/film mode: hide overlapping desk tabs; show single「推理」page with style + LLM settings.
 * Other tasks: restore DOM blocks and standard five-tab layout.
 */
export function updateNovelDeskMode(editor) {
    const desk = editor?.studioDesk;
    if (!desk) return;
    const key = resolveTaskKey(editor.getTaskKey?.() || "t2v");
    const isNovel = key === "novel" || key === "film";
    const isFilm = key === "film";
    desk.classList.toggle("h3d-desk-novel", isNovel);
    desk.classList.toggle("h3d-desk-film", isFilm);

    const novelTab = desk.querySelector('[data-tab="novel-llm"]');
    const novelPage = desk.querySelector('[data-page="novel-llm"]');
    const styleGrid = desk.querySelector('[data-r="desk-style-grid"]');
    const backendBlock = desk.querySelector('[data-r="ld-novel-move-backend"]');
    const styleHost = desk.querySelector('[data-r="novel-llm-style-host"]');
    const backendHost = desk.querySelector('[data-r="novel-llm-backend-host"]');
    const globalPage = desk.querySelector('[data-page="global"]');
    const directorPage = desk.querySelector('[data-page="director"]');

    // Capture original parents only while still on home pages (not after relocate)
    if (styleGrid && !editor._novelStyleHome && styleGrid.closest('[data-page="global"]')) {
        editor._novelStyleHome = styleGrid.parentNode;
    }
    if (backendBlock && !editor._novelBackendHome && backendBlock.closest('[data-page="director"]')) {
        editor._novelBackendHome = backendBlock.parentNode;
    }

    const hideTabs = ["continuity", "global", "director", "imagedir", "export"];
    for (const tab of hideTabs) {
        const btn = desk.querySelector(`[data-tab="${tab}"]`);
        const page = desk.querySelector(`[data-page="${tab}"]`);
        if (isNovel) {
            ...
            // Visibility for director/imagedir restored by their own updaters; others always show
            if (tab === "continuity" || tab === "global" || tab === "export") {
    ...
    novelTab?.classList.toggle("hidden", !isNovel);
    if (novelPage) {
        novelPage.classList.toggle("hidden", !isNovel);
        novelPage.style.display = isNovel ? "" : "none";
    }

    if (isNovel) {
        _moveDeskBlock(styleGrid, styleHost);
        _moveDeskBlock(backendBlock, backendHost);
        desk.querySelector('[data-r="ld-shot-count-wrap"]')?.classList.add("hidden");
        desk.querySelector('[data-r="ld-shot-range-row"]')?.classList.add("hidden");
        const title = desk.querySelector('[data-r="desk-title"]');
        if (title) title.textContent = isFilm ? "电影模式 · 推理与风格" : "小说短剧 · 推理与风格";
        const meta = desk.querySelector('[data-page="novel-llm"] .h3d-meta');
        if (meta && meta.textContent && meta.textContent.includes("分镜")) {
            meta.textContent = isFilm
                ? "长片操作在上方面板：追加分镜 → 挂图 → 继续生成；此处仅配置 LLM 与风格声景。"
                : "短剧操作在上方面板：本章分镜 / 一键跑全书；此处仅配置 LLM（提取资产/本章分镜）与整体风格声景。";
        }
        // Activate novel-llm tab/page
        desk.querySelectorAll("[data-tab]").forEach((b) => b.classList.toggle("active", b === novelTab));
        desk.querySelectorAll("[data-page]").forEach((p) => {
            const on = p === novelPage;
            p.classList.toggle("active", on);
            if (on) {
                p.classList.remove("hidden");
                p.style.display = "";
        ...
        // Entering novel/film: lock skill to product task
        const entered = editor._lastNovelDeskKey !== key;
        if (entered) {
            const skillId = isFilm ? "novel-film" : "novel-short-drama";
            if (td && typeof td === "object") td.skill_id = skillId;
            const skillEl = desk.querySelector('[data-r="ld-skill"]');
            if (skillEl) skillEl.value = skillId;
            setNodeWidget(editor.node, "local_director_skill", skillId);
        }
        editor._lastNovelDeskKey = key;
    } else {
        editor._lastNovelDeskKey = key;
        if (styleGrid && editor._novelStyleHome) _moveDeskBlock(styleGrid, editor._novelStyleHome);
        if (backendBlock && editor._novelBackendHome) _moveDeskBlock(backendBlock, editor._novelBackendHome);
        // Ensure global/director pages can show again (imagedir/director toggled elsewhere)
        if (globalPage) {
            globalPage.classList.remove("hidden");
            globalPage.style.display = "";
        }
        if (directorPage && taskUsesPromptDirector(key)) {
            directorPage.classList.remove("hidden");
            directorPage.style.display = "";
        }
        if (novelTab?.classList.contains("active") || novelPage?.classList.contains("active")) {
            const fallback = desk.querySelector('[data-tab="continuity"]');
            fallback?.click();
        }
    }
}

/** Show/hide「提示词导演」tab；m2v / novel 强制关闭。 */
export function updatePromptDirectorVisibility(editor) {
    const desk = editor?.studioDesk;
    if (!desk) return;
    const taskKey = editor.getTaskKey?.() || "t2v";
    const key = resolveTaskKey(taskKey);
    if (key === "novel" || key === "film") {
        updateNovelDeskMode(editor);
        return;
    }
