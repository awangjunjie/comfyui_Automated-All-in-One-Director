...
# Hard limits for reference audio / video asset uploads (seconds).
REF_MEDIA_DURATION_MIN_SEC = 2.0
REF_MEDIA_DURATION_MAX_SEC = 15.0


def probe_media_duration_sec(path: str) -> float | None:
    """Return media duration in seconds via ffprobe (audio or video)."""
    import json
    import subprocess
    ...
    try:
        ...
        if dur > 0:
            return dur
    except (subprocess.CalledProcessError, json.JSONDecodeError, OSError, TypeError, ValueError) as exc:
        log.debug("ffprobe format duration failed for %s: %s", path, exc)
...
def assert_ref_media_duration(path: str, *, kind: str = "媒体") -> float:
    """Raise ValueError if duration is outside 2–15 seconds."""
    dur = probe_media_duration_sec(path)
    if dur is None:
        raise ValueError(f"无法读取{kind}时长，请确认文件完整且本机可用 ffprobe")
    lo = REF_MEDIA_DURATION_MIN_SEC
    hi = REF_MEDIA_DURATION_MAX_SEC
    if dur + 1e-3 < lo or dur - 1e-3 > hi:
        raise ValueError(
            f"{kind}时长必须在 {lo:g}～{hi:g} 秒之间（当前约 {dur:.1f} 秒），已禁止上传。"
            "请先裁剪到该范围内再上传。"
        )
    return dur
