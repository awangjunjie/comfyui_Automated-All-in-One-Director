...
# Official OpenAI-format base (docs): https://api.deepseek.com  → POST /chat/completions
# /v1 is accepted but not required; prefer root URL to match curl examples.
...
# Legacy aliases retired after 2026-07-24; map to V4 during transition.
...
def _safe_float(value, default: float = 0.6) -> float:
    if value is None:
...
def default_url_for_format(api_format: str) -> str:
    ...
    if api_format == API_FORMAT_DEEPSEEK:
...
def uses_openai_compat_transport(api_format: str) -> bool:
    """DeepSeek / OpenAI Compatible share /v1/chat/completions transport."""
    return api_format in (API_FORMAT_OPENAI_COMPAT, API_FORMAT_DEEPSEEK, _LEGACY_OPENAI_FORMAT)
...
def deepseek_base(url: str) -> str:
    """Base URL for DeepSeek OpenAI-format API (docs: https://api.deepseek.com)."""
    base = coerce_llm_url(url, default=DEFAULT_DEEPSEEK_URL)
    lower = base.lower()
    for suffix in ("/chat/completions", "/completions", "/models"):
        if lower.endswith(suffix):
            base = base[: -len(suffix)].rstrip("/")
            lower = base.lower()
            break
    # /v1 is optional compatibility path; normalize to root for official curl shape.
    if lower.endswith("/v1"):
        return base[:-3].rstrip("/")
    return base


def normalize_deepseek_model(model: str) -> tuple[str, str | None]:
    """Return (model_id, forced_thinking_type|None) for DeepSeek V4 + legacy aliases."""
    name = coerce_llm_model(model, default=DEFAULT_DEEPSEEK_MODEL).strip()
    low = name.lower()
    if low in _DEEPSEEK_LEGACY_MODEL_MAP:
        mapped = _DEEPSEEK_LEGACY_MODEL_MAP[low]
        # deepseek-chat ≈ non-thinking; deepseek-reasoner ≈ thinking
        forced = "enabled" if low == "deepseek-reasoner" else "disabled"
        return mapped, forced
    if low in {"deepseek-v4-flash", "deepseek-v4-pro"}:
        return low, None
    # Keep unknown ids (custom gateway) as-is.
    return name, None
...
def llm_chat_endpoint(url: str, api_format: str) -> str:
    ...
    if api_format == API_FORMAT_DEEPSEEK:
        return f"{deepseek_base(url)}/chat/completions"
...
def llm_models_endpoint(url: str, api_format: str) -> str:
    ...
    if api_format == API_FORMAT_DEEPSEEK:
        return f"{deepseek_base(url)}/models"
...
def resolve_api_key(api_format: str, widget_key: str = "") -> str:
    ...
    if uses_openai_compat_transport(api_format):
        return (
...
def _apply_openai_generation_options(
    ...
    *,
    ...
    if _is_openai_new_completion_model(model):
        ...
        payload["temperature"] = _safe_float(temperature, 0.6)
    ...
    if api_format == API_FORMAT_DEEPSEEK:
        # DeepSeek docs: thinking.type enabled|disabled (default enabled on V4);
        # director/extract should send disabled unless user opts in.
        think_type = (thinking or "disabled").strip().lower()
        if think_type not in ("enabled", "disabled"):
            think_type = "disabled"
        payload["thinking"] = {"type": think_type}
        if think_type == "enabled":
            # Optional: low | high | max (docs default high)
...
def enhance_prompt_sync(
    *,
    ...
    if api_format == API_FORMAT_DEEPSEEK and not resolve_api_key(api_format, api_key):
        return None, "DeepSeek API Key 未配置（请在面板填写或设置环境变量 DEEPSEEK_API_KEY）"
...
def llm_chat_text_sync(
    *,
    ...
    # DeepSeek V4：旧别名 deepseek-chat/reasoner → v4-flash；默认关闭思考以保证结构化输出
    if api_format == API_FORMAT_DEEPSEEK:
        model, forced_think = normalize_deepseek_model(model)
        if forced_think:
            think_mode = forced_think
        else:
            think_mode = (thinking or "disabled").strip().lower() or "disabled"
        if think_mode == "enabled" and int(max_tokens) < 8192:
            max_tokens = 8192
    ...
    *,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if api_format == API_FORMAT_DEEPSEEK and not resolve_api_key(api_format, api_key):
                return list(DEEPSEEK_FALLBACK_MODELS), "DeepSeek API Key 未配置"
            ...
            async with session.get(
                ...
                if resp.status != 200:
                    ...
                    if api_format == API_FORMAT_ZHIPU:
                        return list(ZHIPU_FALLBACK_MODELS), None
                    if api_format == API_FORMAT_DEEPSEEK:
                        return list(DEEPSEEK_FALLBACK_MODELS), None
            ...
            if not models and api_format == API_FORMAT_ZHIPU:
                return list(ZHIPU_FALLBACK_MODELS), None
            if not models and api_format == API_FORMAT_DEEPSEEK:
                return list(DEEPSEEK_FALLBACK_MODELS), None
            return models, None
    except Exception as exc:
        if api_format == API_FORMAT_ZHIPU:
