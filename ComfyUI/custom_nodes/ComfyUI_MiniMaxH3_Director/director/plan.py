...
class SegmentPlan:
    ...
    novel_shot_index: int | None = None
    tail_frame_file: str = ""
    act_boundary: bool = False
...
def load_reference_tensor(ref: dict) -> torch.Tensor | None:
    if ref.get("imageFile"):
        rel = str(ref["imageFile"]).replace("\\", "/").strip()
        candidates = []
        # Absolute / project paths (novel assets etc.)
        if os.path.isabs(rel) or (len(rel) > 2 and rel[1] == ":"):
            candidates.append(rel)
        # Comfy input-relative
        candidates.append(os.path.join(folder_paths.get_input_directory(), rel.replace("/", os.sep)))
        # Also try under output (novel project mirror)
        try:
            candidates.append(os.path.join(folder_paths.get_output_directory(), rel.replace("/", os.sep)))
        except Exception:
            pass
        ...
        if tensor is not None:
            refs.append(
                SegmentRef(
                    ...
                    label=str(item.get("label") or item.get("roleLabel") or "").strip(),
                    image_file=str(
                        item.get("imageFile")
                        or item.get("image_file")
                        or item.get("inputFile")
...
def _trim_reference_audio(audio: dict | None, item: dict) -> dict | None:
    """Apply optional startSec/endSec clip window on a loaded AUDIO dict."""
    if not audio or not isinstance(audio, dict):
        return audio
    try:
        start_sec = float(item.get("startSec") if item.get("startSec") is not None else item.get("start_sec") or 0)
    except (TypeError, ValueError):
        start_sec = 0.0
    try:
        end_raw = item.get("endSec") if item.get("endSec") is not None else item.get("end_sec")
        end_sec = float(end_raw) if end_raw not in (None, "") else 0.0
    except (TypeError, ValueError):
        end_sec = 0.0
    ...
    if end_sec > start_sec:
        ...
        trimmed = wave[..., start_i:]
    if trimmed is None or int(getattr(trimmed, "shape", [0])[-1] or 0) < 1:
        log.warning("Reference audio clip empty after trim startSec=%s endSec=%s", start_sec, end_sec)
        return None
...
def load_reference_audio_item(item: dict) -> dict | None:
    ...
    if audio is None:
        ...
        return None
    return _trim_reference_audio(audio, item if isinstance(item, dict) else {})
...
def segment_ref_audios_for_context(task_key: str, audios: list[SegmentRefAudio]) -> list[SegmentRefAudio]:
    ...
    if task_key not in {"r2v", "novel", "film", "m2v", "rv2v"}:
        return []
    return audios
...
def _load_ref_videos(
    ...
    fps = float(timeline.get("frameRate") or 24.0) or 24.0
    for item in video_list or []:
        ...
        try:
            start_sec = float(
                item.get("startSec") if item.get("startSec") is not None else item.get("start_sec") or 0
            )
        except (TypeError, ValueError):
            start_sec = 0.0
        try:
        ...
        if end_sec > start_sec:
            count = max(1, min(count, int(round((end_sec - start_sec) * fps))))
        try:
            tensor = load_reference_video_clip(
                item, timeline, count, start_frame=start_frame
            )
        except Exception as exc:
...
def _missing_media_tags(text: str, kind: str, indices: list[int]) -> list[str]:
    """Return tags like <Picture N> that are absent from text (per-index)."""
    raw = text or ""
    lower = raw.lower()
    kind_l = kind.lower()
    missing: list[str] = []
    for i in indices:
        n = int(i) + 1
        if f"<{kind_l} {n}>" in lower:
            continue
        if kind_l == "picture" and (
            f"<\u56fe\u7247 {n}>" in raw or f"<\u56fe\u7247{n}>" in raw
        ):
            continue
...
def reinforce_r2v_prompt(
    ...
    *,
    ...
    pics = _missing_media_tags(text, "Picture", pic_indices)
    if pics:
        prefix_parts.append(" ".join(pics))
    vids = _missing_media_tags(text, "Video", vid_indices)
    if vids:
        prefix_parts.append(" ".join(vids))
    auds = _missing_media_tags(text, "Audio", aud_indices)
    if auds:
        prefix_parts.append(" ".join(auds))
    if not prefix_parts:
        return text
...
def count_all_timeline_segments(timeline_data: str) -> int:
    ...
    if task_key in {"fl2v", "fl_chain"} or str(timeline.get("timelineMode") or "").lower() == "fl2v":
...
def build_director_plan(
    ...
    *,
    ...
    if task_key_early in {"fl2v", "fl_chain"} or str(timeline.get("timelineMode") or "").lower() == "fl2v":
    ...
    for idx, (start, end, seg_data) in enumerate(segment_ranges):
        if edit_mode == "global":
            ...
            # Multi-segment edit: do not let empty prompts silently share one global text
            if not (seg_data.get("prompt") or "").strip() and len(segment_ranges) > 1:
                log.warning(
                    "segment #%d has empty prompt — falling back to global (prefer per-segment prompts)",
                    idx + 1,
                )
        ...
        if seg_prompt:
            log.info("plan segment #%d prompt[:80]=%r", idx + 1, seg_prompt[:80])
        ...
        segments.append(
            SegmentPlan(
                ...
                locations=[
                    str(x).strip()
                    for x in (seg_data.get("locations") or [])
                    if str(x).strip()
                ],
                novel_chapter_id=str(
                    seg_data.get("novelChapterId") or seg_data.get("novel_chapter_id") or ""
                ),
                novel_shot_index=(
                    int(seg_data["novelShotIndex"])
                    if seg_data.get("novelShotIndex") is not None
                    else (
                        int(seg_data["novel_shot_index"])
                        if seg_data.get("novel_shot_index") is not None
                        else None
                    )
                ),
                tail_frame_file=str(
                    seg_data.get("tailFrameFile") or seg_data.get("tail_frame_file") or ""
                ),
                act_boundary=bool(
                    seg_data.get("actBoundary")
                    or seg_data.get("act_boundary")
                    or (
                        # First shot of a novel/film chapter/act
                        (seg_data.get("novelShotIndex") == 0 or seg_data.get("novel_shot_index") == 0)
                        and (
                            seg_data.get("novelChapterId")
                            or seg_data.get("novel_chapter_id")
                        )
                    )
                ),
            )
...
# fl2v uses keyframes; v2v uses source clip as <Video 1>; i2v/r2v use ref_images (pure reference).
CONTEXT_REFERENCE_EXCLUDED_KEYS = frozenset({"fl2v", "fl_chain", "t2v", "v2v"})
...
def plan_summary(plan: DirectorPlan) -> str:
    ...
    if mode in ("gen_blank", "gen_image", "prompt_batch", "image_batch", "fl2v"):
        if mode == "fl2v":
            if plan.global_task_key == "fl_chain":
                mode_label = "链式首尾帧 (fl_chain)"
            elif plan.continuity_enabled:
                mode_label = "首尾帧+链式连贯 (fl2v)"
            else:
                mode_label = "首尾帧 (fl2v)"
