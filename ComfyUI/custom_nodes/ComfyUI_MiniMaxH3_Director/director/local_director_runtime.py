...
# Medium / look tags that must yield to desk_style / STYLE_SKILL on refresh
_MEDIUM_STYLE_RE = re.compile(
    r"(?:"
    r"2\s*[DdＤｄ]|3\s*[DdＤｄ]|二维|三维|"
    r"手绘(?:风|风格)?|赛璐璐|二次元|三次元|"
    r"皮克斯风?|迪士尼风?|定格动画|黏土动画|"
    r"写实(?:电影|摄影|风格)?|动漫风?|动画电影|卡通风?|"
    r"CG(?:动画|渲染)?|油画风?|水彩风?|像素风?"
    r")"
    r"(?:\s*(?:动画|风格|质感|渲染|电影|画面|短片))?"
)
...
def strip_stale_medium_style(text: str) -> str:
    """Remove common medium/look phrases so new desk_style / skill can take over."""
    if not text:
        return ""
    out = _MEDIUM_STYLE_RE.sub("", text)
    out = re.sub(r"[ \t]{2,}", " ", out)
    out = re.sub(r"\n{3,}", "\n\n", out)
    out = re.sub(r"[，,]{2,}", "，", out)
    return out.strip(" \t，,;；\n")
...
def reconcile_prompt_with_desk_style(prompt: str, desk_style: str) -> str:
    """Drop stale medium tags from prompt, then ensure current desk_style is present."""
    style = (desk_style or "").strip()
    text = (prompt or "").strip()
    if style:
        text = strip_stale_medium_style(text)
        if style and style not in text:
            text = f"{text}\n{style}".strip() if text else style
    return text
...
def director_mode_from_task_key(task_key: str) -> str:
    ...
    if key in {"i2v", "i2i"}:
        return "I2VA"
    if key in {"r2v", "novel", "film", "m2v", "r2i", "rv2v", "vi2v", "vrc2v", "v2v", "ads2v", "mv2v"}:
        return "REF2VA"
    return "T2VA"
...
def _append_preserve_blocks(
    ...
    *,
    ...
    style_authority = bool(
        force_style_refresh
        or (desk_style or "").strip()
        or (str(skill_id or "none").strip().lower() not in ("", "none"))
    )
    if desk_bits:
        if style_authority and (desk_style or "").strip():
            lines.append(
                "全局声景台设定（声景/配乐必须保留；画面风格以「整体风格」与 STYLE_SKILL 为准，"
                "若与旧全局或简述冲突，删除旧画风词如 2D/3D/动漫/写实 等并按新风格重写）："
            )
        else:
    ...
    if gp:
        if style_authority:
            gp = strip_stale_medium_style(gp) or gp
            lines.append(
                "已有全局提示词（必须保留角色身份、场景位置与声音要点；"
                "画面风格以 STYLE_SKILL / 整体风格 / 创意简述为准，删除与之冲突的旧画风词后扩写）："
            )
        else:
            lines.append(
                "已有全局提示词（必须保留其中的角色、场景位置、风格与声音要点，在此基础上扩写润色，不要另起炉灶抹掉）："
            )
        lines.append(gp)
        lines.append("")
...
def _parse_global_meta(global_text: str) -> tuple[str, dict[str, Any]]:
    ...
    for line in str(global_text or "").splitlines():
        ...
        if low.startswith("shot_count:") or raw.startswith("分镜数:"):
            ...
            try:
                meta["shot_count"] = max(1, int(float(re.search(r"\d+", part).group(0))))
            except Exception:
                pass
            continue
...
def plan_story_beats(
    ...
    **kwargs,
    ...
    shot_count = max(1, int(shot_count or 2))
...
def _run_cloud(
    *,
    ...
    try:
        temp = 0.6 if temperature is None else float(temperature)
    except (TypeError, ValueError):
        temp = 0.6
    try:
        max_tok = 2048 if max_tokens is None else int(max_tokens)
    except (TypeError, ValueError):
        max_tok = 2048
    try:
    ...
    **kwargs,
    ...
    if backend == "cloud":
        ...
        return _run_cloud(
            model=model,
            user_msg=user_msg,
            **{k: kwargs[k] for k in cloud_keys if k in kwargs and kwargs[k] is not None},
        )
    llama_keys = (
    ...
    return _run_llama(
        model=model,
        user_msg=user_msg,
        **{k: kwargs[k] for k in llama_keys if k in kwargs and kwargs[k] is not None},
    )
...
def expand_brief(
    *,
    ...
    gp_in = (global_prompt or "").strip()
    if force_style_refresh and gp_in:
        gp_in = strip_stale_medium_style(gp_in) or gp_in
    ...
    user_msg = _build_user_message(
        ...
        force_style_refresh=bool(force_style_refresh),
    )
...
def expand_prompt_groups(
    ...
    **kwargs,
    ...
    unless force_style_refresh is set (style/skill changed).
    """
    if not groups:
    ...
    kw = dict(kwargs)
    force_style = bool(kw.pop("force_style_refresh", False))
    gp_in = (global_prompt or "").strip()
    if force_style and gp_in:
        gp_in = strip_stale_medium_style(gp_in) or gp_in
    if desk_style:
        gp_in = reconcile_prompt_with_desk_style(gp_in, desk_style)
    ...
    for i, g in enumerate(groups):
        ...
        if _looks_expanded_prompt(source) and not force_style:
            kept.append({
                "index": idx,
        ...
        if brief_only and not _looks_expanded_prompt(brief_only):
            ...
            brief = source
        elif force_style and source:
            brief = strip_stale_medium_style(source) or source
        to_expand.append({
            **g,
            "brief": brief,
            "index": idx,
    ...
    if not to_expand:
        if kept:
            return {
                "shots": kept,
                "global_prompt": gp_in or (global_prompt or "").strip(),
    ...
    for i, g in enumerate(to_expand):
        ...
        prompt = expand_brief(
            ...
            continuity=continuity,
            story_context=story,
            global_prompt=gp_in or g_prompt,
            desk_style=desk_style,
            desk_soundscape=desk_soundscape,
            desk_music=desk_music,
            force_style_refresh=force_style,
    ...
    if not g_prompt.strip():
        ...
        try:
            g_prompt = expand_brief(
                model=model,
                brief=story or (gp_in or global_prompt or "整片全局设定"),
                mode=mode,
            ...
            g_prompt = gp_in or global_prompt or ""
    if desk_style or force_style:
        g_prompt = reconcile_prompt_with_desk_style(g_prompt, desk_style)
...
def _build_story_auto_message(
    *,
    ...
    lo = max(1, int(shot_min))
    hi = max(lo, int(shot_max))
    d_lo = max(0.5, float(duration_min))
    d_hi = max(d_lo, float(duration_max))
    mode = _normalize_mode(mode)
    lines = [
        f"MODE={mode}",
        f"SHOT_COUNT_MIN={lo}",
...
def expand_story_auto(
    ...
    **kwargs,
    ...
    if not brief:
        raise ValueError("提示词导演：创意简述为空")
    lo = max(1, int(shot_min or 2))
    hi = max(lo, int(shot_max or 8))
    d_lo = max(0.5, float(duration_min or 2.0))
    d_hi = max(d_lo, float(duration_max or 12.0))
