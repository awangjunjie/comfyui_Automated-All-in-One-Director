...
CONTINUITY_TASK_KEYS = frozenset({"fl2v", "fl_chain", "i2v", "r2v", "novel", "film", "m2v", "t2v"})
...
def resolve_prev_segment_output(
    ...
    if cached is not None:
        return cached
    # Film/novel resume: load persisted last-frame PNG from previous shot
    tail = str(getattr(prev_seg, "tail_frame_file", "") or "").strip()
    if tail:
        from .film_resume import load_tail_frame_tensor

        loaded = load_tail_frame_tensor(tail)
        if loaded is not None:
            completed[prev_idx] = loaded
            return loaded
    ...
    raise ValueError(
        ...
        "若使用「选择运行」，请确保上一段已有有效缓存或电影模式已保存尾帧。"
    )
