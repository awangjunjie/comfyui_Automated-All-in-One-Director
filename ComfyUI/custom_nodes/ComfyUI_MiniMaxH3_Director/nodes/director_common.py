...
from ..director.studio_enrich import ensure_studio_fields, enrich_timeline_json
...
def prepare_director_plan(
    *,
    ...
    # Inject Continuity board / camera / desk / run_scope before planning.
    try:
        timeline_data = enrich_timeline_json(timeline_data)
        # Keep widget global_prompt in sync when desk/continuity rewrote global.prompt
        try:
            enriched = json.loads(timeline_data)
            ensure_studio_fields(enriched)
            gp = ((enriched.get("global") or {}).get("prompt") or "").strip()
            if gp:
                global_prompt = gp
        except Exception:
            pass
    except Exception as exc:
        log.warning("Studio enrich skipped: %s", exc)
    ...
    if export_segments or (is_batch and not video_batch):
        ...
        if export_segments:
            report = (
                ...
                + "(CreateVideo/SaveVideo should run once per clip)."
            )
            if len(segment_outputs) <= 1:
                report = (
                    report
                    + "\nOnly 1 clip was produced — check that the timeline has multiple "
                    + "prompt groups/shots, and that「选择运行」did not leave a single group."
                )
